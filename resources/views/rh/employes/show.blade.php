...
.tab-btn { padding:8px 16px; border-radius:8px; font-size:13px; font-weight:600; background:#f1f5f9; color:#64748b; cursor:pointer; border:none; }
        ...
        <div class="row g-2">
            ...
            <div class="col-6">
                <div style="background:white;border-radius:10px;padding:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);border-top:3px solid #1d4ed8;text-align:center;">
                    <div style="font-size:18px;font-weight:800;color:#1d4ed8;">{{ number_format($dernierBulletin?->net_a_payer ?? 0, 0, ',', ' ') }}</div>
    ...
    <div class="col-md-8">
        <div class="info-card">
            <div class="d-flex gap-2 mb-3 flex-wrap">
                ...
                <button class="tab-btn" onclick="showTab('t-paie', this)">💰 Bulletins</button>
                <button class="tab-btn" onclick="showTab('t-abs', this)">🗓️ Absences</button>
                <button class="tab-btn" onclick="showTab('t-pret', this)">🏦 Prêts</button>
                <button class="tab-btn" onclick="showTab('t-sanc', this)">⚠️ Sanctions</button>
            ...
            <div id="t-info" class="tab-panel active">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="info-row"><span>Situation</span><span>{{ $employe->situation_matrimoniale }}</span></div>
                        <div class="info-row"><span>Nb enfants</span><span>{{ $employe->nb_enfants }}</span></div>
                        <div class="info-row"><span>Santé</span><span>{{ $employe->etat_sante }}</span></div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="info-row"><span>Niveau acad.</span><span>{{ $employe->niveau_academique ?? '-' }}</span></div>
                        <div class="info-row"><span>Spécialité</span><span>{{ $employe->specialite_academique ?? '-' }}</span></div>
                        <div class="info-row"><span>Diplôme recrut.</span><span>{{ $employe->diplome_recrutement ?? '-' }}</span></div>
                        <div class="info-row"><span>Exp. précédente</span><span>{{ $employe->exp_poste_precedent ?? '-' }}</span></div>
                    </div>
                </div>
            </div>
            ...
            <div id="t-paie" class="tab-panel">
                <table style="width:100%;font-size:12px;border-collapse:collapse;">
                    <thead><tr style="background:#1e3a5f;color:white;">
                        <th style="padding:8px;">Période</th><th>Vague</th><th>Brut</th><th>Net</th><th>Statut</th><th>PDF</th>
                    </tr></thead>
                    <tbody>
                    @forelse($employe->bulletins as $b)
                        <tr style="border-bottom:1px solid #e2e8f0;">
                            ...
                            <td><span style="font-size:10px;padding:2px 8px;border-radius:8px;background:{{ $b->statut==='payé'?'#dcfce7':($b->statut==='validé'?'#fef3c7':'#f1f5f9')}};color:{{ $b->statut==='payé'?'#15803d':($b->statut==='validé'?'#92400e':'#475569')}};">{{ $b->statut }}</span></td>
                            <td><a href="{{ route('rh.paie.pdf', $b->id) }}" class="btn btn-danger" style="font-size:10px;padding:2px 8px;">📄</a></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center text-muted py-3">Aucun bulletin</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div id="t-abs" class="tab-panel">
                <table style="width:100%;font-size:12px;border-collapse:collapse;">
                    <thead><tr style="background:#1e3a5f;color:white;">
                        <th style="padding:8px;">Référence</th><th>Type</th><th>Début</th><th>Fin</th><th>Jours</th><th>Statut</th>
                    </tr></thead>
                    <tbody>
                    @forelse($employe->absences as $a)
                        <tr style="border-bottom:1px solid #e2e8f0;">
                            ...
                            <td><span style="font-size:10px;padding:2px 8px;border-radius:8px;background:{{ $a->statut==='approuvé'?'#dcfce7':($a->statut==='refusé'?'#fee2e2':'#fef9c3')}};color:{{ $a->statut==='approuvé'?'#15803d':($a->statut==='refusé'?'#b91c1c':'#854d0e')}};">{{ $a->statut }}</span></td>
                        </tr>
                    @empty
            ...
            <div id="t-pret" class="tab-panel">
                <table style="width:100%;font-size:12px;border-collapse:collapse;">
                    <thead><tr style="background:#1e3a5f;color:white;">
                        <th style="padding:8px;">Type</th><th>Montant</th><th>Remboursé</th><th>Restant</th><th>Statut</th>
                    </tr></thead>
                    <tbody>
            ...
            <div id="t-sanc" class="tab-panel">
                <table style="width:100%;font-size:12px;border-collapse:collapse;">
                    <thead><tr style="background:#1e3a5f;color:white;">
                        <th style="padding:8px;">Date</th><th>Type</th><th>Motif</th><th>Montant</th><th>Statut</th>
                    </tr></thead>
                    <tbody>
